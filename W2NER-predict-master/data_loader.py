...
def process_bert(data, tokenizer, vocab):
    ...
    for index, instance in enumerate(data):
        ...
        for entity in instance["ner"]:
            ...
            # if index[0] != 0:
            #     _grid_labels[index[-1], index[0]-1] = vocab.label_to_id(entity["type"])
            # if index[0] != len(_grid_labels) - 1:
            #     _grid_labels[index[-1], index[0]+1] = vocab.label_to_id(entity["type"])
...
def load_data_bert(config):
    ...
    #tokenizer = BartTokenizer.from_pretrained("/root/anaconda3/MyProjects/PyTorch_BERT_PRC_Biaffine_NER-main/pretrained_model")
    # tokenizer = AutoTokenizer.from_pretrained(config.bert_name)
    tokenizer = AutoTokenizer.from_pretrained(config.bert_name, cache_dir="./cache/")
    # tokenizer = BertTokenizer.from_pretrained("/root/anaconda3/MyProjects/PyTorch_BERT_PRC_Biaffine_NER-main/pretrained_model")
