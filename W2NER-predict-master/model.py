...
class MultiHeadSelfAttention(nn.Module):
    def __init__(self, hid_size, num_heads, dropout=0.1):
        super().__init__()
        self.num_heads = num_heads
        self.attention = nn.MultiheadAttention(embed_dim=hid_size, num_heads=num_heads, dropout=dropout)
        self.dropout = nn.Dropout(dropout)
        
    def forward(self, x, mask=None):
        # x shape: (seq_len, batch_size, hid_size)
        x = x.permute(1, 0, 2)  # 转换为 (seq_len, batch_size, hid_size)
        attn_output, _ = self.attention(x, x, x, key_padding_mask=mask)
        attn_output = self.dropout(attn_output)
        return attn_output.permute(1, 0, 2)  # 转回 (batch_size, seq_len, hid_size)
...
class IDCNN(nn.Module):
    def __init__(self, input_size, hidden_size, kernel_size, dilation_sizes, dropout=0.1):
    ...
    def forward(self, x):
        ...
        x = self.input_proj(x)  # 投影到隐藏层维度

        for layer in self.layers:
            x = F.relu(layer(x))

        x = x.transpose(1, 2)  # 转换回 [batch_size, seq_len, hidden_size]
...
class Model(nn.Module):
    def __init__(self, config):
        ...
        lstm_input_size = 0
        # bertconfig = BertConfig.from_pretrained("/root/anaconda3/MyProjects/PyTorch_BERT_PRC_Biaffine_NER-main/pretrained_model")
        # self.bert = BertModel(bertconfig)
        self.bert = AutoModel.from_pretrained(config.bert_name, cache_dir="./cache/", output_hidden_states=True).to("cuda:0")
        ...
        self.self_attention = MultiHeadSelfAttention(config.lstm_hid_size, num_heads=8, dropout=config.emb_dropout)

    def forward(self, bert_inputs, grid_mask2d, dist_inputs, pieces2word, sent_length):
        
        # print("bert_inputs: ", bert_inputs.shape)
        ...
        # 使用 IDCNN 处理特征
        # idcnn_outputs = self.idcnn(bert_embs)  # [batch_size, seq_len, conv_hid_size]
        ...
        # 加入多头注意力
        # word_mask = pieces2word.sum(dim=-1).gt(0)  # [8, 107]
        # attention_output = self.self_attention(word_reps, mask=~word_mask)
        # alpha = 0.001
        # word_reps = alpha * attention_output + (1 - alpha) * word_reps
