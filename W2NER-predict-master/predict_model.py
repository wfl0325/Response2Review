...
class LayerNorm(nn.Module):
    def __init__(self, input_dim, cond_dim=0, center=True, scale=True, epsilon=None, conditional=False,
        ...
        if self.center:
            self.beta = nn.Parameter(torch.zeros(input_dim)).to("cuda:0")
        if self.scale:
            self.gamma = nn.Parameter(torch.ones(input_dim)).to("cuda:0")

        if self.conditional:
            if self.hidden_units is not None:
                self.hidden_dense = nn.Linear(in_features=self.cond_dim, out_features=self.hidden_units, bias=False).to("cuda:0")
            if self.center:
                self.beta_dense = nn.Linear(in_features=self.cond_dim, out_features=input_dim, bias=False).to("cuda:0")
            if self.scale:
                self.gamma_dense = nn.Linear(in_features=self.cond_dim, out_features=input_dim, bias=False).to("cuda:0")
...
class Model(nn.Module):
    def __init__(self, config):
        ...
        self.bert = AutoModel.from_pretrained(r"/root/autodl-tmp/bert-chinese-base",
                                              cache_dir=r"./models/W2NER/baseline/cache",
                                              output_hidden_states=True).to("cuda:0")
        lstm_input_size += config.bert_hid_size

        self.dis_embs = nn.Embedding(20, config.dist_emb_size).to("cuda:0")
        self.reg_embs = nn.Embedding(3, config.type_emb_size).to("cuda:0")
        ...
        self.dropout = nn.Dropout(config.emb_dropout).to("cuda:0")
